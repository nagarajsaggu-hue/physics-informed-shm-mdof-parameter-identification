"""
evaluate_stage2.py — Post-training evaluation for Stage 2 Blind 2DOF PINN

Run this AFTER trainer_stage2.py has completed.
...
Usage:
  ...
  python evaluate_stage2.py --start 2 --end 4
...
def save_evaluation_report(eval_summary: pd.DataFrame, stage2_root: Path):
    ...
    lines = [
        "# Stage 2 PINN — Evaluation Report\n",
...
def evaluate_run(run_id: int, paths: dict, df_full: pd.DataFrame) -> Optional[dict]:
    ...
    if not model_path.exists():
        ...
        print(f"             Fix: python -m mdof_2dof_pinn.stage2.trainer_stage2 "
              f"--start {run_id} --end {run_id} --force")
        return None

    # ── Load saved config (with fallback to metrics.csv) ────────────────────
    init_params = None
    true_params = {}

    if cfg_path.exists() and cfg_path.stat().st_size > 10:
        try:
            with open(cfg_path, encoding="utf-8") as f:
                cfg_snap = json.load(f)
            init_params = cfg_snap.get("init_parameters")
            true_params = cfg_snap.get("true_parameters", {})
        except (json.JSONDecodeError, KeyError):
            print(f"  [Run {run_id}] ⚠  config.json corrupt — falling back to metrics.csv")

    # Fallback: derive init_params from metrics.csv true values (use true as init proxy)
    if init_params is None:
        metrics_path = run_dir / "metrics.csv"
        if not metrics_path.exists():
            print(f"  [Run {run_id}] ✗  Neither config.json nor metrics.csv found. "
                  f"Re-run trainer with: --start {run_id} --end {run_id} --force")
            return None
        mrow = pd.read_csv(metrics_path).iloc[0].to_dict()
        true_params = {p: float(mrow[f"{p}_true"]) for p in
                       ["m1","m2","k1","k2","alpha","beta"]
                       if f"{p}_true" in mrow}
        # Use init values from metrics if available, else fall back to true values
        init_params = {p: float(mrow.get(f"{p}_init", mrow[f"{p}_true"]))
                       for p in ["m1","m2","k1","k2","alpha","beta"]
                       if f"{p}_true" in mrow}
        # Add omega/zeta to true_params
        for p in ["omega1","omega2","zeta1","zeta2"]:
            if f"{p}_true" in mrow:
                true_params[p] = float(mrow[f"{p}_true"])
        print(f"  [Run {run_id}] ⚠  Using metrics.csv as config fallback")

    # ── Rebuild model and load weights ───────────────────────────────────────
    model = ScaledTwoDOFPINN(init_params=init_params).to(device)
    try:
        state = torch.load(model_path, map_location=device, weights_only=False)
        model.load_state_dict(state)
    except EOFError:
        print(f"  [Run {run_id}] ✗  model.pt is empty/corrupt (training crashed before saving).")
        print(f"             Fix: python -m mdof_2dof_pinn.stage2.trainer_stage2 "
              f"--start {run_id} --end {run_id} --force")
        return None
    except (RuntimeError, Exception) as e:
        print(f"  [Run {run_id}] ✗  Could not load model: {e}")
        print(f"             Fix: python -m mdof_2dof_pinn.stage2.trainer_stage2 "
              f"--start {run_id} --end {run_id} --force")
        return None
    model.eval()
    print(f"  [Run {run_id}] ✓  Model loaded  ({model_path.stat().st_size/1024:.1f} KB)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Post-training evaluation for Stage 2 Blind 2DOF PINN"
    )
    ...
    parser.add_argument("--skip_missing", action="store_true",
                        help="Silently skip runs whose model.pt is missing or corrupt")
    ...
    print("=" * 80)
    print("STAGE 2 EVALUATION — Post-training analysis")
    print("=" * 80)
    ...
    for run_id in run_list:
        ...
        try:
            ...
            import traceback
            print(f"  [Run {run_id}] ✗  Unexpected error: {exc}")
            traceback.print_exc()
            summary = None
        if summary:
    ...
    if not all_summaries:
        print("\nNo runs evaluated successfully.")
        print("Check that trainer_stage2.py completed without errors.")
        print("Re-run trainer with --force to regenerate model.pt files.")
        return 1
    ...
    # ── Markdown report ───────────────────────────────────────────────────────
    save_evaluation_report(eval_df, paths["stage2_root"])
    ...
    print(f"  Report          : {paths['stage2_root']}/evaluation_report.md")
    print("=" * 80)
    return 0
